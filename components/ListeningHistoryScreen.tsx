...
interface ListeningHistoryScreenProps {
  sessions: ListeningSession[];
  onNewSession: () => void;
}

const ListeningHistoryScreen: React.FC<ListeningHistoryScreenProps> = ({ sessions, onNewSession }) => {
  ...
  return (
    <div className="animate-fade-in">
      <div className="bg-slate-800 rounded-xl shadow-lg p-6 sm:p-8 mb-8">
        <h2 className="text-2xl font-bold text-cyan-300 mb-4">Your Listening Progress (% Correct)</h2>
        {sessions.length > 1 ? (
          ...
          <p className="text-slate-400 text-center py-10">Complete at least two listening tests to see your progress chart.</p>
        )}
      </div>
      ...
      <div className="bg-slate-800 rounded-xl shadow-lg p-6 sm:p-8">
        ...
        <div className="space-y-4">
          {sessions.length > 0 ? (
            ...
            <p className="text-slate-400 text-center py-10">You haven't completed any listening tests yet.</p>
          )}
        </div>
      </div>
