...
import { ListeningReport } from '../types';
import { CheckCircleIcon, XCircleIcon } from './Icons';

interface ListeningReportScreenProps {
  report: ListeningReport;
  onNewSession: () => void;
  onHistory: () => void;
}

const ListeningReportScreen: React.FC<ListeningReportScreenProps> = ({ report, onNewSession, onHistory }) => {
  return (
    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      <div className="text-center bg-slate-800 p-8 rounded-xl shadow-lg">
        <h2 className="text-2xl font-bold text-white mb-2">Your Listening Test Report</h2>
        ...
        <div className="space-y-4">
            {report.results.map((result) => (
                <div key={result.questionId} className="bg-slate-900 p-4 rounded-lg border-l-4 border-slate-700">
                    ...
                    <div className={`flex items-center gap-3 p-2 rounded-md ${result.isCorrect ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                        {result.isCorrect ? <CheckCircleIcon className="w-5 h-5 text-green-400 flex-shrink-0" /> : <XCircleIcon className="w-5 h-5 text-red-400 flex-shrink-0" />}
                        <p className="text-slate-300">Your answer: <span className="font-medium">{result.userAnswer || `(No answer)`}</span></p>
                    </div>
                    {!result.isCorrect && (
                        <div className="flex items-center gap-3 p-2 mt-2 rounded-md bg-slate-700/50">
...
export default ListeningReportScreen;
