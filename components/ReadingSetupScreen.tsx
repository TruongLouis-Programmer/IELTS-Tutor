...
import { generateReadingTest, generateReadingTopic } from '../services/geminiService';
import { ReadingIcon, HistoryIcon, LoaderIcon, TimerIcon, SparklesIcon } from './Icons';

interface ReadingSetupScreenProps {
  onStart: (test: ReadingTest, time: number) => void;
  onHistoryClick: () => void;
  hasHistory: boolean;
}
...
const ReadingSetupScreen: React.FC<ReadingSetupScreenProps> = ({ onStart, onHistoryClick, hasHistory }) => {
  ...
  const handleGenerateTopic = async () => {
    ...
    try {
      const newTopic = await generateReadingTopic();
      setTopic(newTopic);
    } catch (error) {
  ...
  const handleStartPractice = async () => {
    ...
    try {
      const newTest = await generateReadingTest(topic, difficulty, questionCount);
      onStart(newTest, timeLimit * 60);
    } catch (error) {
  ...
  return (
    <div className="bg-slate-800 rounded-xl shadow-lg p-6 sm:p-8 flex flex-col items-center animate-fade-in">
      <ReadingIcon className="w-12 h-12 text-cyan-400 mb-4" />
      <h2 className="text-2xl font-bold text-cyan-300 mb-4 text-center">Prepare for Your Reading Test</h2>
      <p className="text-slate-400 mb-8 text-center max-w-2xl">
        Generate or enter a topic, then customize your test settings.
      </p>
      ...
      <div className="w-full max-w-2xl space-y-8 mb-8">
        ...
        {/* Time Limit Selector */}
        <div>
            <label className="flex items-center justify-center text-lg font-medium text-slate-300 mb-3">
...
export default ReadingSetupScreen;