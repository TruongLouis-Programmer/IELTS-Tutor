...
interface SpeakingHistoryScreenProps {
  sessions: SpeakingSession[];
  onNewSession: () => void;
}

const SpeakingHistoryScreen: React.FC<SpeakingHistoryScreenProps> = ({ sessions, onNewSession }) => {
  ...
  const chartData = sessions
    ...
    .map(session => ({
      ...
      band: session.feedback.overallBand,
    }));

  return (
    <div className="animate-fade-in">
      <div className="bg-slate-800 rounded-xl shadow-lg p-6 sm:p-8 mb-8">
        <h2 className="text-2xl font-bold text-cyan-300 mb-4">Your Speaking Progress</h2>
        {sessions.length > 1 ? (
          ...
          <p className="text-slate-400 text-center py-10">Complete at least two speaking sessions to see your progress chart.</p>
        )}
      </div>
              ...
              <div key={session.id} className="bg-slate-900 p-4 rounded-lg cursor-pointer hover:bg-slate-700 transition-colors" onClick={() => setSelectedSession(session)}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-2xl font-bold text-cyan-400">{session.feedback.overallBand.toFixed(1)}</p>
                  </div>
                </div>
              </div>
            ))
          ) : (
            <p className="text-slate-400 text-center py-10">You haven't completed any speaking sessions yet.</p>
          )}
        </div>
      </div>
      ...
      {selectedSession && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4" onClick={() => setSelectedSession(null)}>
          <div className="bg-slate-800 rounded-xl shadow-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 animate-fade-in" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-cyan-300 mb-2">Session Details</h3>
            ...
            <div className="space-y-4 text-slate-300">
                ...
                <div className="bg-slate-900 p-4 rounded-lg">
                    <h4 className="font-semibold text-slate-200 mb-2">Transcript (Band: {selectedSession.feedback.overallBand.toFixed(1)}):</h4>
                    <div className="whitespace-pre-wrap max-h-60 overflow-y-auto break-words space-y-2">
                        {selectedSession.transcript.map((t, i) => (
                            <p key={i}><strong>{t.speaker === 'user' ? 'You' : 'Examiner'}:</strong> {t.text}</p>
                        ))}
                    </div>
                </div>
                 <div className="bg-slate-900 p-4 rounded-lg">
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      {selectedSession.feedback.areasForImprovement.map((item, index) => <li key={index}>{item}</li>)}
                    </ul>
                </div>
            </div>
